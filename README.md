...
A lightweight Python library and CLI for extracting structured **Trust & Safety signals** from text. Rules-based, fast, fully auditable, and zero external dependencies.
...
- **Harm tags**: inferred category labels (`harassment`, `phishing`, `doxxing`, `threat`, `spam`, `coordinated`, `impersonation`, and more)
- **Flags**: named high-signal findings for fast triage (`THREAT_LANGUAGE_DETECTED`, `SSN_PATTERN_DETECTED`, `PHISHING_PATTERN_DETECTED`, and more)
...
**Python 3.8+. No external dependencies.** Nothing to install; run it straight from the repo root.
...
```bash
# Analyze inline text
python -m content_signals.cli "You better watch your back. I know where you live @user123."

# Analyze a file
python -m content_signals.cli --file message.txt

# JSON output
python -m content_signals.cli --file message.txt --format json

# Save report to file
python -m content_signals.cli --file message.txt --output report.txt

# Flags only (for pipeline use)
python -m content_signals.cli --file message.txt --flags-only

# Read from stdin
echo "Your text here" | python -m content_signals.cli --stdin
```
...
**Rules-based by design.** No ML model, no API calls, no external dependencies. Every signal is traceable to a specific pattern in `patterns.py`, so you can read, audit, and modify the logic directly. This makes it suitable for production pipelines where explainability and latency matter.
...
└── content_signals/
    ...
    ├── extractor.py      # Core extraction engine: extract()
...
**Use as a pipeline pre-processor:** The `--flags-only` CLI flag outputs one flag per line, which is easy to pipe into downstream tooling:

```bash
python -m content_signals.cli --file message.txt --flags-only | grep "THREAT\|PHISHING"
```
